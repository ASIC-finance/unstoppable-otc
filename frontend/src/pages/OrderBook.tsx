...
export function OrderBook() {
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold text-white mb-4">Order Book</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
        <div>
          ...
          <input
            type="text"
            value={tokenA}
            onChange={e => { setTokenA(e.target.value); setPage(0) }}
            placeholder="0x..."
        ...
        <div>
          ...
          <input
            type="text"
            value={tokenB}
            onChange={e => { setTokenB(e.target.value); setPage(0) }}
            placeholder="0x..."
            className="w-full px-3 py-2 rounded-lg bg-gray-900 border border-gray-700 text-gray-100 placeholder-gray-500 text-sm focus:border-emerald-500 focus:outline-none"
          />
        </div>
      ...
      {addrA && addrB && !pairExists && (
        <p className="text-gray-500 text-center py-8">
          No pair exists for these tokens yet. Create an order to deploy one.
        </p>
      )}
      ...
      {pair && (
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm text-gray-400">
            Pair: {token0 && <TokenBadge address={token0} />}
            <span>/</span>
      ...
      {pair && !isLoading && orders.length === 0 && (
        <p className="text-gray-500 text-center py-8">No active orders in this pair.</p>
      )}
      ...
      {!addrA || !addrB ? (
        <p className="text-gray-500 text-center py-8">Enter two token addresses to view a pair's order book.</p>
      ) : null}
    </div>
  )
}
